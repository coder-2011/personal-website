...
export const onRequest = defineMiddleware(async ({ request, url, locals }, next) => {
  ...
  if (!['GET', 'HEAD'].includes(request.method) || !locals.runtime?.env.BLOG_BUCKET ||
  ...
  try {
    ...
    const response = redirectBlogAlias(request, entries) || (request.method === 'HEAD' ? await next() : await cachedBlogResponse({
