...
**Edit details** opens the saved title, date, and description in full-width fields. Choose **Review changes**, then **Update post**. The review includes the latest saved note as well as its public details. Live Sync keeps the setting shown in the dialog. The update button spins until the server accepts the changes. Cancel leaves the saved details untouched; validation and network failures keep your entries available for retry. If Live Sync changes the post after review, review again before updating.

To change an existing URL, choose **Edit details → Change URL**, enter a new lowercase URL ending, then **Review changes**. The review shows both full addresses. Type the new URL ending exactly before **Change URL and update** becomes available. Editing the draft again resets that confirmation; cancelling leaves the old URL intact. The same confirmation is required when republishing a previously unpublished note, and every changed URL in a batch must be confirmed separately.

Old article links and live-update URLs redirect to the latest address. Previous URLs stay reserved, including after unpublishing, so another post cannot take over an old link; they cannot be reused as new URLs. A stale review cannot overwrite a newer revision, and an already-open plugin using an old URL follows the current address on its next accepted save. Unpublishing removes access through every address.
